...
void search_aligned_BLAST(std::istream& FASTQ_file, Rcpp::StringVector aligned, const char * output_path)
{
	remove(output_path);
	std::string line, 
				seq_id;
...
void search_aligned_BLAST_gz(gzFile& FASTQ_file, Rcpp::StringVector aligned, const char * output_path)
{
	remove(output_path);
...
Rcpp::CharacterVector aligned_BLAST_sequences(
	...
	{ 
		...
		search_aligned_BLAST_gz(FASTQ_file, aligned, output_path);
		gzclose(FASTQ_file);
		...
		search_aligned_BLAST(FASTQ_file, aligned, output_path);
	}
	return(0);
}
