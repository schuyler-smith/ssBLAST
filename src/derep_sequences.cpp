...
void process_seqs(std::istream& FASTQ_file, const char * output_path, bool fasta)
{
	remove(output_path);
	...
	for (size_t i = 0; i < sorted_seq_counts.size(); ++i)
	{
		if(fasta){
...
void process_seqs_gz(gzFile& FASTQ_file, const char * output_path, bool fasta)
{
	remove(output_path);
	std::string line, seq_id, seq;
	int seq_line = 1;
...
int derep_sequences(
	...
	if(strcmp(ext.c_str(), ".gz") == 0) { 
		...
		process_seqs_gz(FASTQ_file, output_file, fasta);
		gzclose(FASTQ_file);
		...
		process_seqs(FASTQ_file, output_file, fasta);
	}
	return(0);
}
